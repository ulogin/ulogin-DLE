    ...
    function __construct() {
        ...
        if ($this->userIsLogged) {
            ...
            if (
                ...
                && md5($member_id['password']) == $_SESSION['dle_password']
            ) {
...
//=================================================================================
    ...
    protected function regUser(){
        ...
        for($i = 0; $i < 11; $i ++) {
            $password .= $salt{mt_rand( 0, 72)};
        }

        $password = password_hash($password, PASSWORD_DEFAULT);
        $password = $this->model->safesql($password);
        $this->upass = md5($password);

        $hash = '';

        for($i = 0; $i < 9; $i ++) {
            $hash .= $salt{mt_rand( 0, 72 )};
        }
